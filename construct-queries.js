...
function constructListDecisionsQuery (batchSize, graph) {
  ...
  let query = fs.readFileSync('./queries/1-list-decisions.sparql', { encoding: 'utf8' });
...
function constructDecisionToTreatmentQuery (graph, decisionUris) {
  ...
  let query = fs.readFileSync('./queries/2-decision-to-treatment.sparql', { encoding: 'utf8' });
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
...
function constructLinkReportNewsletterQuery (graph, decisionUris) {
  ...
  let query = fs.readFileSync('./queries/3-link-report-newsletter.sparql', { encoding: 'utf8' });
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
  query = query.replace('# VALUES_PLACEHOLDER', decisionUris.map(sparqlEscapeUri).join('\n    '));
  return query;
}
...
function constructListWithoutTreatmentQuery (batchSize, graph) {
  ...
  let query = fs.readFileSync('./queries/4-list-items-without-treatment.sparql', { encoding: 'utf8' });
  query = query.replace('# LIMIT_PLACEHOLDER', batchSize);
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
  return query;
}
...
function constructAttachTreatmentToOtherItemVersionsQuery (graph, agendaItemUris) {
  ...
  let query = fs.readFileSync('./queries/5-attach-treatment-to-other-versions.sparql', { encoding: 'utf8' });
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
...
function constructAttachTreatmentsToSubcase (graph) {
  ...
  let query = fs.readFileSync('./queries/6-attach-treatment-to-subcase.sparql', { encoding: 'utf8' });
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
  return query;
}
...
function constructAttachExistingNliQuery (graph) {
  ...
  let query = fs.readFileSync('./queries/7-attach-existing-newsletterinfo-to-behandeling.sparql', { encoding: 'utf8' });
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
  return query;
}
...
function constructSelectAnnouncementsWithoutNliQuery (graph) {
  ...
  let query = fs.readFileSync('./queries/8-select-announcements-without-nli.sparql', { encoding: 'utf8' });
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
...
function constructSelectNliForAnnouncementsQuery (agendaItemUri, graph) {
  ...
  let query = fs.readFileSync('./queries/9-select-newsletterinfo-for-announcements.sparql', { encoding: 'utf8' });
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
...
function constructMigrateStatusCodeQuery (graph) {
  ...
  let query = fs.readFileSync('./queries/10-decision-resul-code-migration.sparql', { encoding: 'utf8' });
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
  return query;
}
...
function constructDateMigrationQuery(graph) {
  ...
  let query = fs.readFileSync('./queries/11-treatements-date-types.sparql', { encoding: 'utf8' });
  query = query.replaceAll('# GRAPH_PLACEHOLDER', sparqlEscapeUri(graph));
  return query;
}
